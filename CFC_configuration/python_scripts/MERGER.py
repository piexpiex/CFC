...
print('Procesing CFC_catalogs catalogs')
...
catalogs=os.listdir('catalogs_folder/CFC_sources')
print('Procesing CFC_sources catalogs')
print('Reading catalogs')
...
for j in range(len(catalogs)):
	...
	if catalogs[j][len(catalogs[j])-4:len(catalogs[j])]=='fits':
		if medidor==0:
			hdu11= fits.open('catalogs_folder/CFC_sources/'+catalogs[j])
			nrows = hdu11[1].data.shape[0]
			medidor=1
		else:
			hdul2=fits.open('catalogs_folder/CFC_sources/'+catalogs[j])
			nrows2 = hdul2[1].data.shape[0]
			nrows = nrows + nrows2
			if medidor==2:
				hdu = fits.BinTableHDU.from_columns(hdu2.columns, nrows=nrows)
...
warnings.filterwarnings("ignore")
sys.stdout = open(os.devnull, 'w')
hdu.writeto('catalogs_folder/merge_catalogs/total_sources.fits',overwrite=True)
votable2=Table.read('catalogs_folder/merge_catalogs/total_sources.fits')
votable2.write('catalogs_folder/merge_catalogs/total_sources.xml',table_id='table_id',format='votable',overwrite=True)
sys.stdout = sys.__stdout__
